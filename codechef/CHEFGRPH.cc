...
void update(ll n1, ll m1, ll n2, ll m2, ll n, ll m, ll nl, ll ml) {
    ...
    if (n2 == nl) {
	layer[m2] = (layer[m2] + ((edge.find(mp(n1, m1)) != edge.end())?(edge.find(mp(n1, m1))->second):((n1 > 0) ? mypow(m, n1 - 1) : 1)))%MOD;
    } else {
	for (ll i = 0; i < m; ++i) {
	    sum = (sum + layer[i])%MOD;
	}
	...
	if (n2 != n + 1) {
	    for (ll i = 0; i < m; ++i) {
		layer[i] = (mypow(m, (n2 - nl - 1))*sum)%MOD;
	    }
	} else {
	    layer[0] = (mypow(m, (n2 - nl - 1))*sum)%MOD;
	}

	if (n1) {
	    layer[m2] = (layer[m2] + ((edge.find(mp(n1, m1)) != edge.end())?(edge.find(mp(n1, m1))->second):mypow(m, n1 - 1)))%MOD;
	} else {
	    layer[m2] = (layer[m2] + 1)%MOD;
	}
    }
...
int main() {
	...
	if (nl != n) {
	    for (ll i = 0; i < m; ++i) {
		layer[i] = (mypow(m, (n - nl - 1))*sum)%MOD;
	    }
